<script type="text/javascript">
	...
	$('.remove_problem').click(function () {
		if(confirm("Really? You want to remove this problem from this contest?")) {
			...
			var url = "<?php echo site_url($module.'/'.$this->subview.'/remove_problem'); ?>/" + problemid + '/' + contestid;
	...
	$('.remove_judge').click(function () {
		if(confirm("Really? You want to remove this judge from this contest?")) {
			...
			var url = "<?php echo site_url($module.'/'.$this->subview.'/remove_judge'); ?>/" + judgeid + '/' + contestid;
			$.post( url, function( data ) {
				if(data == 'yes') {
					$('#'+id).fadeOut(1000);
				}
	...
	$('.remove_contestant').click(function () {
		if(confirm("Really? You want to remove this contestant from this contest?")) {
			...
			var contestid = $(this).attr('contestid');
			var id = 'user' + userid;
			var url = "<?php echo site_url($module.'/'.$this->subview.'/remove_contestant'); ?>/" + userid + '/' + contestid;
			$.post( url, function( data ) {
				if(data == 'yes') {
					$('#'+id).fadeOut(1000);
				}
	...
	$('.judge_reply_form').submit(function() {
		var url = "<?php echo site_url($module.'/contest/reply_clar'); ?>";
	...
	$('#judge_clarification').submit(function() {
		var url = "<?php echo site_url($module.'/contest/judge_clar'); ?>";
  	...
  	$('.delete_clar').click(function() {
		var url = "<?php echo site_url($module.'/contest/delete_clar'); ?>";
	...
	$('.view_submission').click(function() {
		...
		var url = "<?php echo site_url($module.'/'.$this->subview.'/fetch_submission'); ?>/" + subid;
		$.post( url, function( data ) {
			if(data != '') {
				data = "<pre>" + data + "</pre>";
